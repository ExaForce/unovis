...
export class XYContainer<Datum> extends ContainerCore {
  ...
  public setData (data: Datum[], preventRender?: boolean): void {
    ...
    if (config.yAxisSecondary && config.yAxisSecondary.datamodel.data !== data) config.yAxisSecondary.setData(data)
  ...
  public updateContainer (containerConfig: XYContainerConfigInterface<Datum>, preventRender?: boolean): void {
    ...
    if (containerConfig.yAxisSecondary) this.config.yAxisSecondary.config.type = AxisType.Y
    ...
    this._reconcileChildren([
      ...
      containerConfig.yAxisSecondary?.element,
  ...
  protected _preRender (): void {
    ...
    // Pass size, margin, and the color function to the components
  ...
  private _updateScalesDomain<T extends XYComponentCore<Datum>> (...components: T[]): void {
    ...
    const secondaryY = components.filter(c => this._isSecondaryY(c))
    if (!secondaryY.length) {
    ...
    // Components on the secondary Y scale get their own domain; the primary domain uses the rest
    const primaryY = components.filter(c => !this._isSecondaryY(c))
    const primaryYWithDomain = componentsWithDomain.filter(c => !this._isSecondaryY(c))
    const secondaryYWithDomain = componentsWithDomain.filter(c => this._isSecondaryY(c))
    this._applyScaleDomain(ScaleDimension.Y, primaryYWithDomain, primaryY, config.yDomain, config.yDomainMinConstraint, config.yDomainMaxConstraint)
    this._applyScaleDomain(ScaleDimension.Y, secondaryYWithDomain, secondaryY, config.ySecondaryDomain, undefined, undefined)
  }
