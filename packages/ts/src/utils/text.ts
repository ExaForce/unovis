...
export function getWrappedText (
  ...
  const textWrapped: Array<string[]> = textArrays.map(block => {
    ...
    const isTrimmed = maxLines && (lines.length > maxLines || hasLinesWiderThan(block, lines, width, fastMode))
    const fittedBlock = isTrimmed
      ? { ...block, text: trimTextToFitLines(block, width, fastMode, separator, wordBreak, maxLines, trimMode) }
      : block

    if (balance && width) return breakTextIntoBalancedLines(fittedBlock, width, fastMode, separator, wordBreak)
    return isTrimmed ? breakTextIntoLines(fittedBlock, width, fastMode, separator, wordBreak) : lines
  })
