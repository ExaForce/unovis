...
### Multi-Series Gradient Areas

To apply individual gradients to each series in a multi-area chart, define one `<linearGradient>` per series and reference them by index:

export const seriesColors = ['#3b82f6', '#ef4444', '#f59e0b']
export const multiSeriesDefs = seriesColors.map((color, i) => `
...
<XYWrapper
  ...
  showAxes
  height={200}
  containerProps={{ svgDefs: multiSeriesDefs }}
/>
