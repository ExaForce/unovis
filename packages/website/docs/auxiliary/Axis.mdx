...
### Tick Label Max Lines
Wrapped tick labels can take as many lines as they need by default. Set `tickTextMaxLines` to cap them:
longer labels get trimmed to fit the lines, according to `tickTextTrimType`.
<XYWrapperWithInput
  {...axisProps()}
  inputType="range"
  ...
  hiddenProps={{gridLine: false, x: d => d.timestamp, tickFormat: d=> new Date(d).toDateString(), tickTextTrimType: "end"}}
  defaultValue={2}
  property="tickTextMaxLines"/>

### Balanced Tick Labels
A wrapped tick label fills its lines greedily, so it can end with a short line. Set `tickTextBalanced` to `true` to even
out the lines instead, the way CSS `text-wrap: balance` does: the label keeps its number of lines, but they break at the
narrowest width that still fits them.
<XYWrapperWithInput
  {...axisProps()}
  inputType="checkbox"
  tickTextWidth={80}
...
is limited or when displaying many ticks. Labels closer than `tickTextOverlapTolerance` pixels (`0` by default)
count as overlapping; a negative value lets them overlap by that much.
...
#### Uniform Time Ticks
On time scales the "nice" d3 tick sets can be uneven: the 2-day sets reset at month boundaries, producing sequences like _Jul 27, Jul 29, Jul 31, Aug 1_, and the default multi-scale formats label the month start differently from its neighbors. Set `tickTextAdaptiveSets` to `'uniform'` to place the labels over a calendar-unit grid (seconds, minutes, hours, days, weeks, months or years) with a constant step instead, so they stay evenly spaced across month and year boundaries. On units with a natural cycle the step snaps to the cycle's divisors or whole multiples (e.g. 1/2/3/4/6/12 hours), and the labels prefer cycle-aligned positions (_:00, :15, :30_ rather than _:05, :20, :35_). On non-time scales, and with explicit `tickValues`, `'uniform'` behaves like `'nice'` (an alias of `true`).

The chosen calendar unit is passed to `tickFormat` as the fourth argument, so all the labels can share one date format matching the tick resolution:

```ts
tickFormat: (tick, i, ticks, timeUnit) =>
  timeUnit === 'day' ? formatDayLabel(tick) : formatDefaultLabel(tick)
```
