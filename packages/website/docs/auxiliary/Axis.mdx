...
`labelTextSeparator` property with a `string` or `string[]` of separators. By default the label breaks on spaces and hyphens.

In the example below the label has no spaces or hyphens, so with the default separators it can't wrap and overflows the axis.
