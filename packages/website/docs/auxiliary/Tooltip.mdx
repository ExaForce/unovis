...
The return value controls what the tooltip does:
...
| `undefined` / no return | Leave the content unchanged |

:::tip

Returning `undefined` (or nothing) leaves the tooltip content untouched. Use this when managing
content externally — for example, with React's `createPortal` to render a component directly
into the tooltip element.
